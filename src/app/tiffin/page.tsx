...
export default function TiffinTracker() {
  ...
  const handleTiffinUpdateNumber = (number: number) => {
    if (!selectedDay) return;
  ...
  return (
    <div className="p-5 bg-gray-100 min-h-screen flex flex-col items-center">
      ...
      {/* Clear Count Button */}
      <button onClick={handleClearData} className="mt-4 bg-yellow-500 text-white px-4 py-2 rounded-md shadow-md">
        Clear Count
      </button>

      {/* Modal for Selecting Tiffin Count */}
      {isModalOpen && selectedDay && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            <h2 className="text-lg font-bold mb-4">Select Tiffin Count for {selectedDay}</h2>
            <div className="flex gap-2 justify-center">
              <button onClick={() => handleTiffinUpdateNumber(1)} className="bg-green-500 text-white px-4 py-2">1</button>
              <button onClick={() => handleTiffinUpdateNumber(2)} className="bg-blue-500 text-white px-4 py-2">2</button>
              <button onClick={handleDecrement} className="bg-gray-400 text-white px-4 py-2 rounded-md">-</button>
              <input type="number" min="3" max="5" value={tiffinCount} readOnly className="border p-2 w-20 text-center rounded-md bg-gray-200 cursor-not-allowed" />
              <button onClick={handleIncrement} className="bg-gray-400 text-white px-4 py-2 rounded-md">+</button>
            </div>
            <div className="flex gap-4 mt-4 justify-center">
              <button onClick={handleTiffinUpdate} className="bg-blue-500 text-white px-4 py-2 rounded-md">Save</button>
              <button onClick={() => setIsModalOpen(false)} className="bg-red-500 text-white px-4 py-2 rounded-md">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isClearModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            <h2 className="text-lg font-bold mb-4">Are you sure you want to clear the data?</h2>
            <div className="flex gap-4 justify-center">
              <button onClick={confirmClearData} className="bg-green-500 text-white px-4 py-2 rounded-md">Yes</button>
              <button onClick={() => setIsClearModalOpen(false)} className="bg-red-500 text-white px-4 py-2 rounded-md">No</button>
            </div>
          </div>
        </div>
      )}
